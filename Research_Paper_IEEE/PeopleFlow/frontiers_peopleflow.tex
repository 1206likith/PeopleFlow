...
\def\firstAuthorLast{Kunchi {et~al.}}
\def\Authors{Likith Sai Kunchi\,$^{1}$, Manivannan Thulasirangan Lakshmidevi\,$^{2}$, K S K Karthik Reddy\,$^{3,*}$}
\def\Address{$^{1}$School of Computer Science and Engineering (SCOPE), VIT (Vellore Institute of Technology), Vellore, Tamil Nadu, India \\
$^{2}$School of Civil Engineering, VIT (Vellore Institute of Technology), Vellore, Tamil Nadu, India \\
$^{3,*}$School of Civil Engineering, VIT (Vellore Institute of Technology), Vellore, Tamil Nadu, India}
\def\corrAuthor{K S K Karthik Reddy}
\def\corrEmail{konala.karthik@vit.ac.in}
\makeatletter
\def\@address{}
\def\@correspondance{}
\makeatother
...
\title[PeopleFlow]{PeopleFlow: A Reproducible Geometry-Aware Multi-Agent Framework for Evacuation Safety Evaluation in Smart Building Environments}
...
\begin{abstract}
\section{}
Evaluating evacuation performance in complex buildings requires analysis methods that capture interactions between spatial configuration, pedestrian behavior, and operational disruptions. This paper introduces PeopleFlow, a layout-informed computational framework for structured evacuation-safety comparison across diverse architectural layouts. The workflow converts floor-plan representations into navigable simulation environments and applies continuous-space multi-agent modeling based on force-driven pedestrian dynamics. PeopleFlow reports interpretable indicators including total clearance duration, localized congestion intensity, bottleneck persistence, and exit-utilization balance. To enable consistent experimentation, the framework uses parameterized scenario configurations and reproducibility-oriented execution records. The evaluation covers a structured matrix of nine layout types across five operational conditions (450 core runs), plus 210 controlled supplementary/sensitivity runs for real-layout generalization and sensitivity to exit width, desired speed, and occupancy scaling. Results show strong dependence of evacuation outcomes on geometric topology and routing strategy, with clearance-time differences exceeding an eightfold range and substantial congestion variation across layouts. The primary contribution is a reproducible engineering framework for relative safety prioritization across heterogeneous architectural configurations, rather than deterministic prediction of individual trajectories. The framework supports what-if analysis during early design and provides a computational basis for integration with digital twin workflows for high-occupancy safety planning. From a built-environment perspective, the framework enables early-stage design evaluation of evacuation efficiency and supports decision-making in smart building planning, infrastructure safety assessment, and digital-twin-based emergency management systems.

\tiny
\keyFont{ \section{Keywords:} Crowd simulation, evacuation analytics, evacuation modelling, digital twin, multi-agent modelling, multi-agent simulation, smart infrastructure, building evacuation safety, reproducible pipeline. }
\end{abstract}
...
\begin{tabular}{L{2.35cm} L{2.20cm} L{2.70cm} L{3.15cm}}
\toprule
...
Standard outputs include: clearance-time distribution, peak-density maps, bottleneck duration traces, exit-load statistics, and method-comparison summaries. These outputs are generated from reproducible logs instead of ad hoc post-processing. These indicators are selected to align with practical evacuation-safety assessment metrics used in building design and fire-safety engineering, enabling direct interpretation of simulation outputs for engineering decision-making.
...
\begin{tabular}{>{\bfseries\raggedright\arraybackslash}L{3.20cm} L{7.40cm}}
\toprule
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{L{3.35cm} C{2.45cm} C{2.25cm} C{2.55cm}}
\toprule
...
\begin{tabular}{L{2.35cm} L{4.70cm} L{2.55cm} L{3.00cm}}
\toprule
\textbf{Workflow} & \textbf{Human steps} & \textbf{Operator time} & \textbf{Automated preprocessing} \\
\midrule
PeopleFlow & Upload and optional exit verification & 1--3 min & 0.51--67.58 s (mean 27.01 s) \\
Manual authoring baseline & Geometry tracing, exit placement, and scenario setup & 30--120 min (operator dependent) & Not automated \\
\bottomrule
\end{tabular}
}
\end{table}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.82\textwidth]{fig_fp_conversion_steps.png}
...
\begin{figure}[t]
\centering
\includegraphics[height=0.22\textheight,keepaspectratio]{fig_floorplan.png}
...
\begin{figure}[t]
\centering
\includegraphics[height=0.22\textheight,keepaspectratio]{fig_simulation.png}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.06}
\begin{tabular}{>{\bfseries\raggedright\arraybackslash}L{5.10cm} L{7.10cm}}
\toprule
...
The large variation across layouts highlights the dominant influence of spatial topology compared to behavioral parameter variation alone. From a design perspective, these results highlight that corridor-dominated layouts with constrained connectivity significantly increase evacuation time and congestion risk, whereas open-plan configurations with distributed exits provide more robust evacuation performance under both nominal and stress conditions.

\begin{table*}[t]
\centering
\caption{Comprehensive Results Across Nine Layouts and Five Scenarios (Part A: Acad.--Mall; 10 Runs per Configuration). Peak-density values are kernel-level hotspot intensity indicators for relative ranking and are not direct real-world persons/m$^2$ counts.}
\label{tab:results_main}
{\scriptsize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{0.94}
\begin{tabular}{L{1.55cm}L{1.80cm}R{1.70cm}R{1.35cm}R{1.95cm}R{1.35cm}}
\toprule
...
\begin{table*}[t]
\centering
\caption{Comprehensive Results Across Nine Layouts and Five Scenarios (Part B: P1--Supermkt.; 10 Runs per Configuration).}
\label{tab:results_main_partb}
{\scriptsize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{0.94}
\begin{tabular}{L{1.55cm}L{1.80cm}R{1.70cm}R{1.35cm}R{1.95cm}R{1.35cm}}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{L{3.10cm}C{2.15cm}C{3.15cm}C{1.70cm}}
\toprule
\textbf{Method} & \shortstack{\textbf{Clearance}\\\textbf{(s)}} & \shortstack{\textbf{Peak Density}\\\textbf{(agents/m$^2$)}} & \shortstack{\textbf{Completed}\\\textbf{(\%)}} \\
\midrule
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.06}
\begin{tabular}{>{\bfseries\raggedright\arraybackslash}L{3.10cm} L{2.30cm} L{3.10cm} L{3.20cm}}
\toprule
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{L{4.10cm}C{2.35cm}C{2.20cm}C{1.85cm}}
\toprule
...
The nonlinear reduction in evacuation time with increasing exit width suggests the presence of critical threshold widths beyond which congestion effects are significantly alleviated, indicating potential design optimization points for building egress systems.
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{C{2.65cm}C{2.35cm}C{4.30cm}}
\toprule
...
\begin{tabular}{L{4.30cm} C{2.35cm} C{2.35cm}}
\toprule
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{L{4.30cm} C{2.10cm} C{2.30cm} C{2.10cm}}
\toprule
...
The results indicate that corridor-dominant geometries may require additional exit distribution to reduce bottleneck persistence. Open-hall layouts demonstrate more balanced exit utilization under congestion-aware routing strategies. The proposed framework can be used to compare different layout options during the design stage and to identify configurations that may lead to congestion or delayed evacuation. It may also be useful for testing the impact of changes such as exit placement, corridor width, or occupancy levels. In this way, the approach provides a simple way to explore “what-if” scenarios without requiring detailed manual simulation setup.
...
\section{Conclusion}
This paper presented a reproducible geometry-aware multi-agent framework for structured evaluation of building evacuation safety performance and the following conclusions are drawn: 

1. Across a 450-run core matrix and supplementary parametric/generalization experiments, we showed substantial sensitivity of evacuation outcomes to geometric topology and routing assumptions, including an observed 8.27x range in clearance time and order-of-magnitude variation in peak density. 

2. Ablation, parameter-sensitivity, and method-comparison experiments demonstrate that density-sensitive path-selection policies and scenario design materially affect measured safety outcomes in structured safety-evaluation settings.

3. The work advances evacuation research by framing simulation as an auditable, reproducible framework rather than a one-off prediction tool. The resulting computational workflow supports transparent comparison for architects, safety engineers, and smart-city planners who need evidence-based prioritization of safer design and policy options. 

4. Overall, PeopleFlow provides a practical foundation for experiment-driven evacuation safety evaluation, supports data-driven infrastructure design, and serves as a pre-deployment evidence layer for standards-aligned review, design iteration, and operational preparedness assessment.

5. PeopleFlow supports early-stage evacuation planning by enabling rapid evaluation of architectural design alternatives prior to construction. The framework can assist safety engineers in identifying congestion-prone regions, evaluating exit placement strategies, and testing emergency routing policies within digital twin environments. 

6. Such simulation-driven design assessment can contribute to safer high-occupancy infrastructure including transportation hubs, hospitals, and commercial facilities. The workflow aligns with emerging digital twin methodologies for safety-aware infrastructure modeling.

\section{Appendix: Digital Twin Integration Schema and API Hooks}
...
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.05}
\begin{tabular}{>{\bfseries\raggedright\arraybackslash}L{3.30cm} L{8.70cm}}
\toprule
...
Likith Sai Kunchi: Conceptualization, Methodology, Software, Data Curation, Formal Analysis, Visualization, Writing -- original draft. Manivannan Thulasirangan Lakshmidevi: Validation, Investigation, Writing -- review and editing. K S K Karthik Reddy: Conceptualization, Supervision, Methodology Review, Project Administration, Writing -- review and editing.
