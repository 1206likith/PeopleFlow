...
\begin{IEEEkeywords}
Multi-agent simulation, evacuation modeling, crowd safety, building evacuation, Digital Twins, Smart Building Safety, Multi-agent Pathfinding.
\end{IEEEkeywords}
...
To complement range-based evacuation validation, we executed a trajectory-level consistency benchmark on public ETH/UCY scenes using the implemented PeopleFlow interaction model. Following established trajectory-benchmark practice \cite{pellegrini2009eth,alahi2016sociallstm,gupta2018socialgan,salzmann2020trajectron,mohamed2020stgcnn}, we evaluated two scenes (\texttt{seq\_eth}, \texttt{seq\_hotel}) and report root-mean-square error (RMSE), average displacement error (ADE), and final displacement error (FDE). As summarized in Table \ref{tab:eth_validation}, the aggregate RMSE is 0.628 with ADE 0.516 and FDE 0.542 over 13,953 trajectory samples.
...
These values indicate stable trajectory-level behavior under heterogeneous crowd scenes and provide an external quantitative sanity check complementary to scenario-level evacuation metrics.
...
The PeopleFlow codebase, scenario configurations, and reproducibility scripts are publicly maintained at \url{https://github.com/1206likith/PeopleFlow}. ETH/UCY benchmark inputs are sourced from the public ETH archive used in this study (\url{https://data.vision.ee.ethz.ch/cvl/aem/ewap_dataset_full.tgz}). A DOI-minted archival snapshot (code, configuration manifests, and paper-result artifacts) is prepared for camera-ready release to support long-term reproducibility.
...
The authors used GitHub Copilot and ChatGPT for coding assistance and language polishing during manuscript preparation. The authors take full responsibility for the content, methodology, and technical accuracy of this work.
