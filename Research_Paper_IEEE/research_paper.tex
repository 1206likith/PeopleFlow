...
PeopleFlow is intended to complement, not replace, commercial evacuation tools by prioritizing reproducibility and comparative benchmarking under a transparent experiment workflow.
...
\centering
\begin{tikzpicture}[
layerbox/.style={draw, rounded corners=1.2pt, minimum width=0.82\columnwidth, minimum height=0.70cm, font=\scriptsize, align=center},
...
\draw[vlink] (1.85,1.45) -- (1.85,1.25);
\draw[vlink] (1.85,0.55) -- (1.85,0.35);
\node[font=\scriptsize] at (-1.2,1.55) {stairs};
\node[font=\scriptsize] at (1.2,1.55) {elevator};
\end{tikzpicture}
...
The PeopleFlow codebase, scenario configurations, and reproducibility scripts are publicly maintained at \url{https://github.com/1206likith/PeopleFlow}. ETH/UCY benchmark inputs are sourced from the public ETH archive used in this study (\url{https://data.vision.ee.ethz.ch/cvl/aem/ewap_dataset_full.tgz}). Core paper artifacts can be regenerated through the backend command-line pipeline: from the repository root, enter \texttt{apps/backend} and run \texttt{python -m app.experiments.cli --paper-bundle}, then run \texttt{python -m app.experiments.cli --validate-eth --eth-download} to regenerate ETH outputs. For containerized execution, enter \texttt{infra} and run \texttt{docker compose -f docker-compose.prod.yml up --build}.
